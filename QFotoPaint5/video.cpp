...
{
    VideoCapture cap(0); //Se podría mejorar para que el usuario diga qué cámara pjemplo
    if (cap.isOpened()) {
        Mat frame;
        cap.read(frame); //puede que no devuelva imagen, hay que controlarlo
        int tecla = 0;
        while((tecla=waitKey(1)) == -1 && !frame.empty()) { //no se ha pulsado ninguna tecla
            namedWindow("Pulse una tecla para capturar", 0); //nombre de la ventana
